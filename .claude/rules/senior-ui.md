...
- **`TopBar` 整理（Phase 5）**: 場風/自風/残り山は CenterPanel に集約済みなので `TopBar` からは撤去。残るのは `ドラ`表示・お助け Toggle・設定ボタンの3つだけ。`TopBarProps` から `mySeat` props も削除（自風の表示が無くなったため）。`GameBoard` 側の呼び出しも `mySeat` 渡しなしに。`useMemo` の `dora` と `useSettingsStore` の helpMode/toggle はそのまま使う。
- **鳴き対象牌の点滅**: ロン/ポン/カン/チー が可能なとき、相手の直前捨て牌（`gameState.lastDiscard`）を点滅させる＝「どの牌で鳴けるか」を視覚的に提示。色は付けない（お助けONの黄色 `wait` ハイライトと衝突しないため）＝**お助け非依存の基本UI**。`Tile` に `pulse` props、`DiscardPile` に `pulseLast`（配列末尾の1枚だけ pulse）、`RiverGrid` に `pulseSeat: Seat | null`、`GameBoard` で `canCallNaki = legal && (canRon||canPon||canMinkan||chiOptions.length>0)` から `pulseSeat = canCallNaki ? lastDiscard.seat : null` を導出。鳴きボタンの `animate-pulse` と併せて「ボタン」と「牌」の両方が点滅する。**アニメは Tailwind 標準 `animate-pulse` だとボヤッとして気付かれにくいので、`tailwind.config.ts` に独自 `naki-pulse` keyframes を定義**: `steps(1, end) 0.8s infinite` で opacity 1↔0.15 をカクッと不連続に切替＋scale 1.08↔1 を同期で振る（信号機のような「ON/OFF」の見た目＋「ひょこひょこ」感）。pulse 時の Tile は `relative z-10` も付与＝ scale で隣の牌に被るのを防ぐ。
...
- **牌の立体感（`.tile-shadow` ＋ `<span>` ラッパー）**: 単純な drop-shadow だと「浮いてる」感は出るが「厚みのある板」感は弱いので、`<img>` を `<span class="tile-shadow inline-block rounded-md">` でラップして 3 レイヤーで立体感を作る。**globals.css の `.tile-shadow`**: ① 外側 `box-shadow: 0 6px 10px rgba(0,0,0,.45), 0 2px 3px rgba(0,0,0,.4)`（ambient + contact）でフェルトから浮かせる ② `::after`（`absolute inset-0 border-radius:inherit pointer-events:none`）に `inset 0 1.5px 0 rgba(255,255,255,.7), inset 0 -3px 5px -2px rgba(0,0,0,.28)` で「上端の白いハイライト＋下端の暗い陰」＝牌の厚みの陰影。**hover 持ち上げ**: `.group:not(:disabled):hover > .tile-shadow` で外側影が `0 10px 16px / 0 3px 5px` まで深まる（button 側 `-translate-y-1` と組み合わせて「持ち上げ」感）。`:disabled` の自手牌（合法手でない）は持ち上げない。**ring との衝突回避**: ring（`HIGHLIGHT_CLASSES`＝tenpai-keep/riichi/wait）は `<img>` 側に残し、独自 `box-shadow` は `<span>` 側＝ Tailwind の ring（box-shadow ベース）と衝突しない設計。pulse 中（`animate-naki-pulse`）も scale が span に当たるので外側影＋inset がまとめて拡縮し、自然な「ぴょこぴょこ」になる。

## 音声・効果音の追加メモ
- **デフォルト音量は MAX（1.0）**（`settingsStore.volume`）。シニアのノートPCで本体音量を下げ気味でも聞き取れる前提＋mp3 の素の音量が控えめなのでクランプ上限まで使う。`persist` は `version: 2` の `migrate` で「保存済み volume が 1.0 未満なら 1.0 に上書き」＝既存端末でも実機テスト再開で自動的に大きくなる。これより上げたい場合は HTML `<audio>.volume` のクランプを超えるため Web Audio API の GainNode マスターゲインが必要（未実装）。`SettingsModal` の音量スライダーで個別調整は引き続き可能。
