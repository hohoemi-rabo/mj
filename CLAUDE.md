...
**進捗はチケット単位で `docs/00-index.md` の状態欄を参照（常に最新の真実）。** 現時点のスナップショット: 基盤整備（#01）＋**ゲームロジック層 #02〜#09 完了**（牌`tiles` / 山`wall` / 手牌`hand` / シャンテン`shanten` / 役`yaku`(符込み) / 点数`score` / 進行ステートマシン`state` / CPU`cpu/`(弱中強)、すべて `src/lib/mahjong/` 純粋TS・テスト202件）。**ロジック層は完了**。次は UI/通信側（#10 SVG牌素材・#11 通信・#13 UI基盤 など）。`src/app/page.tsx` などUIページはまだ Create Next App のテンプレートのまま。
